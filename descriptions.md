...
| chpst | Runs a program with a changed process state | None |
...
| crc32 | Compute CRC-32 checksums for the given files | None |
...
| egrep | Fgrep, rgrep - print lines that match patterns | None |
...
| freeramdisk | Frees the memory used by the loadlin ramdisk | None |
...
| getty | Alternative Linux getty | None |
...
| java | Launch a Java application | None |
...
| redis-cli | Command-line client to redis-server | None |
| reformime | MIME E-mail reformatting tool | None |
...
| smemcap | Capture memory usage data for later processing by | None |
| socat | Multipurpose relay | http://www.dest-unreach.org/socat/ |
| softlimit | Runs another program with new resource limits. | None |
...
| tunctl | Create and manage persistent TUN/TAP | None |
